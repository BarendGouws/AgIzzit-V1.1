...
import userKeysMap from '@/utils/userKeysMap';
import { fetchTemplate } from '@/redux/manage/slices/templates';

const CanvasPage = ({ template, setTemplate, width, setWidth, height, setHeight, pageNumber, setPageNumber }) => {
...
const CanvasPDFEditor = ({ docId, isManage }) => { 
  ...
  const handleSaveTemplate = async () => {
    ...
    const recipientPlaceholder = template.placeholders.find(
      (placeholder) => placeholder.role === 'Recipient' && placeholder.type
    );
    ...
    // If validation passes, proceed to make the PUT request
    try {
      ...
      if (response.ok) {
        ...
        errorToast('Failed to save template: ' + result.message);
        console.error('Save error:', result);
      }
    } catch (error) {
      errorToast('An error occurred while saving the template.');
  ...
  const handleDeleteTemplate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to delete template:', data.message);
  ...
  useEffect(() => {
    ...
    const loadTemplate = async () => {
      try {
        const response = await fetch(`/api/templates/${docId}`);       
        if (response.status === 200) {
          const data = await response.json();      
          if (data) { console.log('Loaded template:', data);
            setTemplate(data);    
            ...
            if(data?.fields?.length == 0){
              ...
              const nonProfilePlaceholders = Array.from(presentTags)
                ...
                .map(tag => ({
                  tag,
                  text: templateFields.find(field => field.tag === tag)?.text || 'No description available',
                  isPresent: true
                }));

              setPlaceholderInfo({ profilePlaceholders, nonProfilePlaceholders });
    ...
    docId && dispatch(fetchTemplate(docId));
