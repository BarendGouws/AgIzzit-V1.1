...
// Helper input states for individual tabs
const KeywordForm = memo(({ onAdd }) => {
...
const DescriptionForm = memo(({ onAdd }) => {
...
const NewAdvertisingCampaign = () => {
    ...
    const [campaign, setCampaign] = useState(defaultCampaignState);
    ...
    }, [campaign.campaignType, dispatch]);

    console.log(campaign, "Campaign State");
    ...
    const handleCampaignTypeChange = useCallback((selectedOption) => {
    ...
    const validateStep = useCallback((step) => {
        ...
        if (step === "first") {
            ...
            // Validate manual budget distribution
    ...
    }, [campaign, audienceTargetingValid]);
    ...
    // Search Ad content handlers 
    const addKeyword = useCallback((keywordData) => {
        setCampaign(prev => ({
            ...prev,
            searchAds: {
                ...prev.searchAds,
                keywords: [...prev.searchAds.keywords, {...keywordData}]
            }
        }));
    }, []);
    ...
    const addHeadline = useCallback((text) => {
        setCampaign(prev => ({
            ...prev,
            searchAds: {
                ...prev.searchAds,
                headlines: [...prev.searchAds.headlines, text]
            }
        }));
    }, []);
    
    const removeHeadline = useCallback((index) => {
        setCampaign(prev => ({
            ...prev,
            searchAds: {
                ...prev.searchAds,
    ...
    const addDescription = useCallback((text) => {
        setCampaign(prev => ({
            ...prev,
            searchAds: {
                ...prev.searchAds,
                descriptions: [...prev.searchAds.descriptions, text]
            }
        }));
    }, []);
    ...
    }, [campaign, validateStep]);

    // DemographicsTargeting Component
const DemographicsTargeting = memo(({ campaign, handleInputChange }) => {
    return (
...
// EventCheckbox Component - Memoized to prevent unnecessary re-renders
...
const EventCategory = memo(({ 
    ...
    const isEventSelected = useCallback((eventId) => {
        return audienceSelections[category]?.includes(eventId) || false;
    }, [audienceSelections, category]);
...
// CampaignPurposeOption Component
const CampaignPurposeOption = memo(({ 
    purpose, 
    selectedPurpose, 
    validation, 
    handlePurposeChange,
...
// MyAudienceTargeting Component
const MyAudienceTargeting = memo(({ 
    campaign, 
    handleInputChange, 
    audienceStats, 
    validateNext,
    ...
    // Calculate matched events function
    const calculateMatchedEvents = useCallback((purpose, events) => {
        let runningTotal = 0;
    ...
    const generatePurposeValidation = useCallback((purposes, platforms, eventConfig) => {
        const result = {};
        
        purposes.forEach(purpose => {
            const matchedEvents = calculateMatchedEvents(purpose, eventConfig);
    ...
    // Use useMemo for purposeValidation
    const purposeValidation = useMemo(() => 
        generatePurposeValidation(campaignPurposeOptions, campaign.platforms, eventConfig), 
        [campaign.platforms, eventConfig, campaignPurposeOptions, generatePurposeValidation]
    );
    
    // Check if all purposes are disabled
    const areAllPurposesDisabled = useMemo(() => 
        campaignPurposeOptions.every(option => !purposeValidation[option.value]?.isValid),
        [purposeValidation, campaignPurposeOptions]
    );
    
    // Get platforms that cause issues
    const getProblematicPlatforms = useCallback(() => {
    ...
    }, [campaign.targeting, setAudienceSelections]);
    ...
    }, [selectedPurpose, purposeValidation, validateNext]);
    
    // Seed events based on threshold
    const seedEvents = useCallback((purpose, selectedPlatforms) => {
        if (!purpose) return {};
    ...
    const handlePurposeChange = useCallback((purposeValue) => {
    ...
    }, [selectedPurpose, purposeValidation, campaignPurposeOptions, campaign.platforms, handleInputChange, seedEvents, setAudienceSelections]);
    
    // Calculate total available events based on selected events
    const calculateTotalSelectedEvents = useCallback((selections) => {
        let total = 0;
    ...
    const handleEventSelectionChange = useCallback((category, eventId, isChecked) => {
    ...
    }, [audienceSelections, handleInputChange, setAudienceSelections]);
    
    // Remove a specific platform from the selected platforms
    const handleRemovePlatform = useCallback((platformToRemove) => {
    ...
    const getSelectedEventsCount = useCallback(() => {
                ...
                <Form.Group className="mb-4">
                    ...
                    <div className="campaign-purpose-options">
                        {campaignPurposeOptions.map((purpose) => (
                            <CampaignPurposeOption
                                key={purpose.value}
                                purpose={purpose}
                                selectedPurpose={selectedPurpose}
                                validation={purposeValidation[purpose.value] || {}}
                                handlePurposeChange={handlePurposeChange}
                                handleRemovePlatform={handleRemovePlatform}
                                audienceSelections={audienceSelections}
                                calculateTotalSelectedEvents={calculateTotalSelectedEvents}
                            />
                        ))}
                    </div>
                    ...
                    <Alert variant="warning" className="mb-4">
                        ...
                        {getProblematicPlatforms().length > 0 && (
                            <div className="mt-2">
                                ...
                                {getProblematicPlatforms().map((item) => (
...
// CombinedAudienceTargeting Component
const CombinedAudienceTargeting = memo(function CombinedAudienceTargeting({
    campaign, 
    handleInputChange, 
    audienceStats, 
    validateNext
    ...
    const campaignPurposeOptions2 = [
        { 
            value: 'prospecting', 
            label: 'Prospecting Campaign (Lookalike Audiences)',
            description: 'Target new potential customers who are similar to your existing customers.',            
            allowedCategories: ['conversions', 'intent'],
            minSize: 1000,
        },
        { 
            value: 'midfunnel', 
    ...
    // Handle audience option selection
    const handleAudienceOptionChange = useCallback((selected) => {
                ...
                <Card.Body>
                    ...
                    {selectedAudience === 'demographics' ? (
                        ...
                        <MyAudienceTargeting
                            ...
                            audienceSelections={audienceSelections}
                            setAudienceSelections={setAudienceSelections}
                            campaignPurposeOptions={campaignPurposeOptions}
...
// LocationTargeting Component
const LocationTargeting = memo(({ campaign, handleInputChange, organization }) => {
    ...
    }, [campaign.locations, campaign.targeting.radius, campaign.targeting.locationRadiusValues, handleInputChange]);
    ...
    }, [organization?.locations, campaign.locations.length, handleInputChange]);
    ...
    }, [campaign.targeting.radius, handleInputChange, localRadiusValues]);
    ...
    }, [campaign.targeting.targetType, handleInputChange]);
    ...
    const handleTargetTypeChange = useCallback((selected) => {
    ...
    const handleLocationRadiusChange = useCallback((locationId, value) => {
        setLocalRadiusValues(prev => ({
            ...prev,
    ...
    const handleLocationRadiusChangeComplete = useCallback(() => {
        handleInputChange('targeting.locationRadiusValues', localRadiusValues);
    }, [handleInputChange, localRadiusValues]);
    ...
    }, [organization?.locations, campaign.locations, campaign.targeting.radius, localRadiusValues]);
...
const InventoryFilters = memo(({ campaign, handleInputChange }) => {
    ...
    }, [validateForm, filterForm, conditions, campaign.savedFilters, editingFilter, handleInputChange, initialFormState]);
    ...
    const handleDelete = useCallback((filterId) => {     
        ...
        handleInputChange('savedFilters', updatedFilters);
    }, [campaign.savedFilters, handleInputChange]);

    // Memoized render helpers to reduce re-rendering
    const renderConditionField = useCallback((field, label, error) => (
          ...
          <Col md={3}>
            <Form.Select
              ...
              onChange={(e) => setConditions(prev => ({
                ...prev,
                [field]: { ...prev[field], operator: e.target.value }
              }))}
            >
    ...
    const renderDateConditionField = useCallback((field, label, error) => (
        <Row className="mb-3" key={field}>
            <Col md={4}>
            <Form.Label>{label}</Form.Label>
            </Col>
...
const KeywordList = memo(({ keywords, removeKeyword }) => {
...
const HeadlineList = memo(({ headlines, removeHeadline }) => {
...
const DescriptionList = memo(({ descriptions, removeDescription }) => {
...
// SearchAdContent Component
const SearchAdContent = memo(({ campaign, errors, addKeyword, removeKeyword, addHeadline, removeHeadline, addDescription, removeDescription }) => {
    return (
        <>
            <Card className="mb-4">
                <Card.Header>
                ...
                <Card.Body>
                    ...
                    <div className="mb-3">
                        ...
                        <KeywordList 
                            keywords={campaign.searchAds.keywords} 
            ...
            <Row>
                <Col md={6}>
                    <Card className="mb-4">
                        ...
                        <Card.Body>
                            <div className="mb-3">
                                <HeadlineForm onAdd={addHeadline} />
                                ...
                                <HeadlineList 
                                    headlines={campaign.searchAds.headlines} 
                ...
                <Col md={6}>
                    <Card className="mb-4">
                        ...
                        <Card.Body>
                            <div className="mb-3">
                                <DescriptionForm onAdd={addDescription} />
                                ...
                                <DescriptionList 
                                    descriptions={campaign.searchAds.descriptions} 
...
const TemplateSelection = memo(({ campaign, templates, errors, handleInputChange }) => {
    return (
        <Card className="mb-4">
            <Card.Header>
...
const BudgetSetup = memo(({ campaign, errors, handleInputChange }) => {
    return (
        <Card className="mb-4">
            <Card.Header>
...
// BudgetDistribution Component
const BudgetDistribution = memo(({ campaign, errors, handleInputChange }) => {
    return (
        <Card className="mb-4">
            <Card.Header>
...
// CampaignSchedule Component
const CampaignSchedule = memo(({ campaign, errors, handleInputChange }) => {
    return (
        <Card className="mb-4">
            <Card.Header>
...
const CampaignSummary = memo(({ campaign, campaignTypes, platformOptions }) => {
    return (
        <Card className="mb-4">
            <Card.Header>
