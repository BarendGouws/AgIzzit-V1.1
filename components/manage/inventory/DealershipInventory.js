...
const DealershipInventory  = ({ id, type, router }) => { 
  ...
  const renderAttributeField = (attribute) => {
    ...
    if ((key === 'transmission' || key === 'fuelType') && inventoryItem.subcategory === 'Cars & Bakkies') {
      ...
      const isDisabled = disabled || !inventoryItem.subcategory || (inventoryItem.variant && inventoryItem[key]);
  ...
  const loadDropdownOptions = async (item) => { 
    ...
    if (item?.subcategory === "Cars & Bakkies" && item?.year) {      
      try {
        ...
        const makesRes = await axios.get(`/api/mmcode`, { params: { year: item.year }});
        setMmcodeMakes(makesRes.data);

        if (item.make) {  
          const modelsRes = await axios.get(`/api/mmcode`, { params: { year: item.year, make: item.make }});   
          setMmcodeModels(modelsRes.data);

          if (item.model) {
            const variantsRes = await axios.get(`/api/mmcode`, { params: { year: item.year, make: item.make, model: item.model }});        
            setMmcodeVariants(variantsRes.data);
          }
        }
      } catch (error) {
        console.error("Error loading dropdown options:", error?.response?.data || error.message);
      }
    }
  };
  ...
  const handleYear = async (selectedYear) => {
    ...
    if (selectedYear && inventoryItem.subcategory === "Cars & Bakkies") {
      try {
        const res = await axios.get(`/api/mmcode`, {
          params: { year: selectedYear },
        }); 
        setMmcodeMakes(res.data);
      } catch (error) {
  ...
  const handleMake = async (selectedMake) => {
    ...
    if (selectedMake && inventoryItem.subcategory === "Cars & Bakkies") {
      try {
        const res = await axios.get(`/api/mmcode`, {
          params: { year: inventoryItem.year, make: selectedMake },
        });  
        setMmcodeModels(res.data);
      } catch (error) {
...
DealershipInventory.layout = "ManageLayout";
