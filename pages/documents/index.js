...
const DocumentsPage = () => {
    ...
    const loadSignatures = async (page = 1) => {
        try {
            ...
            errorToast("An error occurred while loading signatures.");   
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleGenerateSignature = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                errorToast(data.error || "Failed to generate signature request.");
            }
        } catch (error) {
    ...
    return (
        <>
            <Seo title="Documents" />
            <Pageheader title="Signatures" heading="Docs" active="Documents" />
                    ...
                    <Col xs={12}>
                        {signatures && signatures.length > 0 ? (
                            signatures.map((signature, index) => (
                                <Link key={index} href={`/documents/${signature._id}`}>
                                    <Card className="mb-2">
                                        <Card.Body>
...
DocumentsPage.layout = "AgIzzitLayout";
