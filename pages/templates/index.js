...
const TemplatesPage = () => {	
    ...
    const handleFileUpload = async () => {
        ...
        try {
            ...
            if (response.ok) {
                successToast('Template uploaded successfully.');
    ...
    const loadTemplates = async (page = 1) => {
        try {
            setLoading(true);
            const response = await fetch(`/api/templates?page=${page}&limit=10${searchTerm ? `&search=${searchTerm}` : ''}`);
            ...
            errorToast("An error occurred while loading templates.");   
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSearch = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/templates?page=1&limit=10&search=${searchTerm}`);
            if (response.status === 200) {
                const data = await response.json();
                if (data) {
                    setTemplates(data.templates);
            ...
            errorToast("An error occurred while searching templates.");   
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <>
            <Seo title={"Templates"} />
            <Pageheader title="Templates" heading="Docs" active="Templates" />	
							...
							{placeholderInfo?.success ? 
								<Card.Body className=" border border-success text-center rounded">
									<div className="success-widget">
										...
										<p className="mt-3 mb-0">Are You Sure?</p>											
