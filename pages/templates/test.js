...
import { Button, Card, Col, Modal, Row, Badge, Alert, Form, Pagination } from "react-bootstrap";
...
const TemplatesPage = () => {	
    ...
    const handleFileUpload = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setPlaceholderInfo(data.placeholderInfo);
                loadTemplates(1);
            } else {
    ...
    const renderPlaceholderInfo = () => {
        ...
        const { profilePlaceholders, nonProfilePlaceholders, availableProfileFields } = placeholderInfo;

        return (
            <div className="mt-3">
                <h6>Placeholder Information:</h6>
                <p>Profile Placeholders: {profilePlaceholders}</p>
                <p>Non-Profile Placeholders: {nonProfilePlaceholders}</p>
                {profilePlaceholders === 0 && (
                    <Alert variant="warning">
                        No profile placeholders were used. Profile placeholders can be used to populate data directly from the recipient's profile. Consider editing your template to include some of these fields:
                        <ul>
                            {availableProfileFields.map((field, index) => (
    ...
    return (
        <Fragment>
            <Seo title={"Templates"} />
            <Pageheader title="Templates" heading="Apps" active="Templates" />	
    
            <Row className="mb-4">					
                <Col xs={12} md={6} className="d-none d-md-block">
                    <div className="fs-18 mb-2">	
                ...
                <Col xs={12} md={6}>
                    <div className="input-group mb-2">
                        <input type="text" className="form-control text-truncate" placeholder="Search templates....." aria-label="Search templates....." aria-describedby="button-addon2" />
                    ...
                    <Col xs={12}>
                        {templates?.length === 0 ? 						
                            ...
                            templates?.map((template, index) => (
                                <Link key={index} href={`/templates/${template._id}`}>
                                    <Card className="mb-2">
                                        <Card.Body>
                                            <Row className="align-items-center">
                                                ...
                                                <Col xs={12} md={2} className="mt-1 mt-md-0">
                                                ...
                                                <Col xs={12} md={2} className="mt-1 mt-md-0">
                                                    <Badge bg="primary" className="me-1">{template.completed ? 'Completed': 'Incomplete'}</Badge>
                                                </Col>																
                                            </Row>
                                        </Card.Body>
                                    </Card>
            ...
            {pagination.totalPages > 1 && (
                <Row className="mt-4">
                    <Col>
                        <Pagination>
                            <Pagination.Prev
                                onClick={() => loadTemplates(pagination.currentPage - 1)}
                    ...
                    <Card className="bd-0 mg-b-20">
                        <Card.Body className="border border-info text-center rounded">
                            <div className="info-widget">
                                ...
                                <h6 className="mt-3 mb-0">The template is used to replace placeholders with actual data.</h6>
                                <p className="mt-3 mb-0">
                                    Please include placeholders in your .docx file. Profile placeholders (e.g., {'{firstName}'}) will be automatically populated from the recipient's profile.
                                </p>
                                <p className="mt-3 mb-0 text-info">							
                                    <strong>The placeholders will be replaced by text at signing</strong>
                                </p>
                            </div>
                        </Card.Body>
                    </Card>
                    <Form.Group controlId="formFile" className="mb-3">
                        <Form.Label>Select a .docx file to upload</Form.Label>
...
TemplatesPage.layout = "AgIzzitLayout";
